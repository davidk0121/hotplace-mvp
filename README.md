...
의도된 한계 (베타 기준):
- 지도: **MapTiler 키가 있으면 실제 MapLibre 지도**, 없으면 미리보기(mock) 폴백
- **좌표는 MapTiler geocoding으로 확정** — 사용자가 후보 2~3개 중 하나를 골라야
  실제 핀이 찍힌다. 위치 없이 저장하면 목록엔 남지만 지도 핀은 안 생긴다
- **주변 제안은 여전히 mock** (정적 좌표 포함) — 실제 라이브 검색 아님
- 데이터는 아직 **localStorage 단일 기기** (Supabase 미연결)
- **소셜(릴스/틱톡) 자동 장소 추출 없음**, bulk import 없음 (향후 단계)

## 실제 지도 설정 (MapTiler, 무료 티어)

앱은 `NEXT_PUBLIC_MAPTILER_KEY` 유무로 동작이 갈린다:
- **키 없음** → 기존 미리보기(mock) 지도로 안전하게 폴백. 앱은 안 깨진다.
- **키 있음** → 실제 MapLibre + MapTiler 지도, 지오코딩 검색, 실제 핀.

키 발급 & 설정:
1. [cloud.maptiler.com](https://cloud.maptiler.com) 무료 가입.
2. **Account → API keys**에서 키 복사.
3. 로컬: `.env.example`을 복사해 `.env.local`을 만들고
   `NEXT_PUBLIC_MAPTILER_KEY=발급받은키` 입력 → `npm run dev` 재시작.
4. Vercel: **Project → Settings → Environment Variables**에
   `NEXT_PUBLIC_MAPTILER_KEY` 추가(Production/Preview) 후 재배포.
5. 보안: `NEXT_PUBLIC_` 값은 클라이언트에 노출되므로 MapTiler 대시보드에서
   **허용 Origin(도메인)을 제한**하는 것을 권장.

무료 티어 한도(대략 월 10만 타일/지오코딩)는 베타 규모엔 충분하다. 지도/위성
전환은 MapTiler `streets-v2`(라이트) / `streets-v2-dark`(다크) / `hybrid`(위성)
스타일을 사용한다. **키·백엔드가 없어도 빌드/배포는 항상 성공한다.**
...
- 배포: Vercel 무료 플랜
- 지도: **MapLibre GL JS + MapTiler 무료 티어**(키 있을 때) / 목업 폴백(키 없을 때).
  장소 카드의 Google/Naver/Kakao "열기" 링크는 그대로 유지
...
- ✅ **Phase A — 실제 지도(MapLibre + MapTiler 무료 티어)**: `MapLibreView`가
  `MapViewProps`를 구현(키 없으면 `MockMap` 폴백), MapTiler geocoding으로
  좌표 후보를 골라 저장, 실제 핀 렌더, 지도/위성 토글, 내 위치 파란 마커.
  `Place`에 `lat/lng` 추가(하위 호환), 지역칩이 실제 지도를 이동시킴.

### 다음 실제 우선순위

1. **Supabase 영속화 + 실제 공유 링크** — `supabase/schema.sql` 실행 후
   `lib/storage.ts` 내부를 Supabase 호출로 교체(+ `places`에 lat/lng 컬럼).
   이 시점에 `/lists/share/[shareId]`가 기기 간 실제 공유 링크로 동작.
2. **URL 메타데이터/이름 자동 채움** — **지도 링크부터** 시작 (Apple/Google
   링크 URL 파싱은 안정적, Vercel API 라우트로), 소셜 링크는 그다음.
3. **Naver/Kakao 지역 검색 하이브리드** — 한국 POI 정확도용 (KR 개발자 계정).
4. **Google/Naver/Kakao 저장 리스트 import** — API 접근이 허용하는 범위에서.
5. **(나중에) 사용자 보조 방식의 릴스/틱톡 장소 후보 추출**:
