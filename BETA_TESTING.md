...
- **자동 입력 기대치**: "링크를 붙여넣으면 장소 이름이 자동으로 채워질 거라고
  기대했어?" (EN: *Did you expect the app to auto-fill the place name from the
  link?*) — 다음 단계(메타데이터 파싱)의 우선순위를 정하는 핵심 질문.
- **미리보기 지도 수용도**: "미리보기 지도, 초기 MVP로는 괜찮았어? 아니면 너무
  가짜같이 느껴졌어?" (EN: *Did the preview map feel acceptable for an early
  MVP, or did it feel too fake?*)

## 현재 MVP의 의도된 한계 (테스터가 물어보면)

- **지도는 미리보기(mock)**: 실제 지도 API 연동 전이라 핀 위치가 지리적으로
  정확하지 않다. 지도 좌하단에 "미리보기 지도 · 실제 위치 연동 예정" 배지가
  떠 있다. 위성 모드도 시각 효과다.
